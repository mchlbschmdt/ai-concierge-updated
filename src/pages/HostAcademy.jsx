...
export default function HostAcademy({ tab }) {
  return (
    <Layout>
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="max-w-md w-full bg-card border border-border rounded-xl p-8 text-center">
          <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-4">
            <span className="text-3xl">{tab === 'progress' ? '📚' : '🎓'}</span>
          </div>
          <h1 className="text-xl font-bold text-foreground mb-2">
            {tab === 'progress' ? 'My Progress' : 'Host Academy'}
          </h1>
          <p className="text-sm text-muted-foreground mb-4">
            {tab === 'progress'
              ? 'Track your learning progress and completed courses.'
              : 'Video training library for short-term rental hosts.'}
          </p>
